...
let baseDonnees = [
  { user: "tp", psw:"tp"},
  { user: "hello", psw:"world"},
  { user: "silia", psw:"taider"},
  { user: "tidal", psw:"tidal"},
  { user: "site", psw:"web"}
];

var tab = [];

function filterAppend(f) {
  filterSelection(f);
  /*if ( !(tab.includes(f))){tab.push(f);}
  filterSelection(tab);
  alert(tab);*/
...
/*function filterSelection(){
  ...
  const filteredDestination = pathologies.filter((ElementPathologie) => {
      return (
          //ElementPathologie.symptomes.toLowerCase().includes(tab[])
          tab.includes(ElementPathologie.type)  
      );
  });
  // displayPathologies(filteredDestination);
  //window.alert(filteredDestination);
...
filterSelection("all")
  ...
  function myFunction2(){
    ...
    const searchString = searchBar.toLowerCase();
        
    const filteredDestination = pathologies.filter((ElementPathologie) => {
        return (
            ElementPathologie.symptomes.toLowerCase().includes(searchString)    
        );
    });
    // displayPathologies(filteredDestination);
    //window.alert(filteredDestination);
...
const displayPathologies = (pathologies) => {
    
    let htmlString = pathologies.map((ElementPathologie) => {
        return "<li class='ElementPathologie'>"+ElementPathologie.title+"</li>";
        })
        .join('');
    let PathologiesList = document.getElementById('Destinations');
    PathologiesList.innerHTML = htmlString;
...
for (const d of pathologies) {					
    let clone = document.importNode(template.content, true);     

    newContent = clone.firstElementChild.innerHTML	
        .replace(/{{title}}/g, d.title)				
        .replace(/{{symptomes}}/g, d.symptomes)
        .replace(/{{meridien}}/g, d.meridien)	
        .replace(/{{caracteristique}}/g, d.caracteristique)				
        .replace(/{{type}}/g, d.type)

        clone.firstElementChild.innerHTML = newContent

        clone.firstElementChild.setAttribute("class", "filterDiv " + d.meridien + " " + d.type + " " + d.caracteristique);
